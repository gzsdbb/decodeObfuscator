function getReDefineFunction(scope, funcNames) {
    ...
    scope.traverse(scope.block, {
        "FunctionDeclaration" (path) {
            ...
            if (!types.isCallExpression(argument) || !types.isIdentifier(argument.callee)) {
                return;
            }
...
function getNewObPaths(path, funcNames) {
    ...
    for (const referPath of referencePaths) {
        ...
        if (AncestorPath.isFunctionDeclaration()) {
            ...
            let { arguments } = node;
            if (arguments.length < 2 || !types.isIdentifier(arguments[0], {
                name: name
...
const CallExpressToLiteral = {
    VariableDeclarator(path) {
        let { scope, node } = path;
        let { id, init } = node;
        if (!types.isArrayExpression(init) || !init.elements.every(element => types.isStringLiteral(element))) {
            return;
        }
        ...
        scope.traverse(scope.block, {
            "CallExpression" (path) {
                ...
                if (!types.isIdentifier(callee) || !funcNames.includes(callee.name) || !isElementsLiteral(path)) {
                    return;
                }
...
function replaceReferNode(newMap, referencePaths, scope) {
    for (const referPath of referencePaths) {
        console.process(referencePaths.length);
...
const deleteObfuscatorCode = {
    VariableDeclarator(path) {
        ...
        if (types.isCallExpression(init)) {
            ...
            if (arguments.length == 0 && sourceCode.includes("apply")) {
                path.remove();
            } else if ((sourceCode.includes("constructor") || sourceCode.includes("RegExp")) && types.isIdentifier(callee) && arguments.length == 2 && types.isThisExpression(arguments[0]) && types.isFunctionExpression(arguments[1])) {
    ...
    ExpressionStatement(path) {
        ...
        if (arguments.length == 2 && types.isThisExpression(arguments[0]) && types.isFunctionExpression(arguments[1])) {
            path.remove();
        }
    },
    CallExpression(path) {
        ...
        if (arguments.length != 2 || !types.isFunctionExpression(arguments[0]) || !types.isNumericLiteral(arguments[1])) {
            return;
        }
    ...
    FunctionDeclaration(path) {
        ...
        if (body.length == 2 && types.isFunctionDeclaration(body[0]) && types.isTryStatement(body[1])) {
            let sourceCode = path.toString();
